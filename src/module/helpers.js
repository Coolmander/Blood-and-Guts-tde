 ...
 */
export const alignDripsGetOffsetAndDimensions = (splats) => {
    ...
    for (let i = 0; i < splats.length; i++) {
        ...
        if (splat.y < lowestY)
            lowestY = splat.y;
        if (splat.x + splat.width > highestX)
            highestX = splat.x + splat.width;
        if (splat.y + splat.height > highestY)
            highestY = splat.y + splat.height;
    }
 ...
 */
export const computeSightFromPoint = (fromPoint, range) => {
    ...
    const sight = canvas.sight.constructor.computeSight(fromPoint, range, minAngle, maxAngle, cullDistance, cullMult, density, walls);
    let lowestX = canvas.dimensions.sceneWidth;
    let lowestY = canvas.dimensions.sceneHeight;
 ...
 */
export const isBnGUpdate = (changes) => {
    const entries = Object.entries(changes);
    if (entries.length === 2 && entries[0][0] === 'flags' && entries[0][1][MODULE_ID] == null)
        return false;
    return true;
};
export const getNestedProp = (theObject, path, separator) => {
    try {
        separator = separator || '.';
        return path
            .replace('[', separator)
 ...
 */
export const drawDebugRect = (container, width = 2, color = 0xff0000) => {
 ...
 * @param {number} [color=0xff00ff] - optional border color.
 */
export const drawDebugRect2 = (x, y, w, h, width = 2, color = 0xff0000) => {
    const rect = new PIXI.Graphics();
 ...
 */
export const getDirectionNrml = (lastPosition, currentPosition) => {
    let x = Number(currentPosition.x > lastPosition.x);
    let y = Number(currentPosition.y > lastPosition.y);
    if (!x)
        x = -Number(currentPosition.x < lastPosition.x);
    if (!y)
        y = -Number(currentPosition.y < lastPosition.y);
 ...
 */
export const getRandomGlyph = (font) => {
 ...
 */
export function getPointOnCurve(p1, pc, p2, t) {
 ...
 */
export function getDerivativeAt(p1, pc, p2, t) {
 ...
 */
export const getRandomBoxMuller = () => {
    let u = 0, v = 0;
    while (u === 0)
    ...
    if (num > 1 || num < 0)
        return getRandomBoxMuller(); // resample between 0 and 1
    return num;
};
/**
...
export const lookupTokenBloodColor = (token) => {
    ...
    else if (!bloodColorEnabled || !BloodNGuts.system)
        return getHexColor('blood');
 ...
 * @returns {string} - color in hex format
 */
export const getColorByActorName = (actor) => {
...
export const generateCustomSystem = (id, type, path) => {
    ...
    return {
        ...
        currentHP: (token, actorType) => {
            const hpPath = BloodNGuts.system.customAttributePaths[BloodNGuts.system.supportedTypes.indexOf(actorType)];
            const hp = getNestedProp(token.actor.data.data, hpPath + '.value');
            return hp;
        },
        maxHP: (token, actorType) => {
            const maxHPPath = BloodNGuts.system.customAttributePaths[BloodNGuts.system.supportedTypes.indexOf(actorType)];
            const maxHP = getNestedProp(token.actor.data.data, maxHPPath + '.max');
            return maxHP;
        },
        currentHPChange: (changes, actorType) => {
            var _a;
            if (!changes)
                return;
            const changeHPPath = BloodNGuts.system.customAttributePaths[BloodNGuts.system.supportedTypes.indexOf(actorType)];
            const changeHP = getNestedProp((_a = changes === null || changes === void 0 ? void 0 : changes.actorData) === null || _a === void 0 ? void 0 : _a.data, changeHPPath + '.value');
            return changeHP;
        },
        maxHPChange: (changes, actorType) => {
            var _a;
            if (!changes)
                return;
            const changeMaxHPPath = BloodNGuts.system.customAttributePaths[BloodNGuts.system.supportedTypes.indexOf(actorType)];
            const changeMaxHP = getNestedProp((_a = changes === null || changes === void 0 ? void 0 : changes.actorData) === null || _a === void 0 ? void 0 : _a.data, changeMaxHPPath + '.max');
            return changeMaxHP;
        },
