...
import { getRandomGlyph, getRandomBoxMuller, alignDripsGetOffsetAndDimensions, getDirectionNrml, getUID, distanceBetween, lookupTokenBloodColor, } from "../module/helpers.js";
import { getBaseTokenSettings } from "../module/settings.js";
/**
...
export default class SplatToken {
    constructor(token) {
        if (!token.id)
            log(LogLevel.ERROR, 'SplatToken constructor() missing token.id');
        this.id = token.id;
        this.token = token;
    ...
    create() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const baseTokenSettings = yield getBaseTokenSettings(this.token);
            const tokenSettingsHandler = {
                get: (target, property) => {
                    if (property === 'bloodColor')
                        return target[property] || this.defaultBloodColor;
                        ...
                        return (target[property] ||
                            game.settings.get(MODULE_ID, 'violenceLevels')[game.settings.get(MODULE_ID, 'masterViolenceLevel')][property]);
                },
                set: (target, property, value) => {
                    target[property] = value;
                    if (property === 'tokenViolenceLevel' && value)
                        target = Object.assign(target, this.violenceLevels[value]);
                    return true;
                },
            };
    ...
    createMask() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const maskTexture = yield loadTexture(this.token.data.img, { fallback: CONST.DEFAULT_TOKEN });
            ...
            this.container.position.set((this.token.data.width * canvas.grid.size) / 2, (this.token.data.height * canvas.grid.size) / 2);
    ...
    get tokenSplats() {
        ...
        if (!history)
            ...
            return history.events.filter((e) => e.tokenId === this.id);
    }
    /**
    ...
    trackChanges(changes) {
        ...
        if (hasProperty(changes, `flags.${MODULE_ID}`)) {
            const settingsUpdates = Object.keys(changes.flags[MODULE_ID]).filter((key) => ['bloodColor', 'floorSplatFont', 'trailSplatFont', 'tokenSplatFont'].includes(key));
        ...
        if (hitSeverity > this.bleedingSeverity)
            this.bleedingSeverity = newBleedingSeverity = hitSeverity;
        else if (hitSeverity <= 0 && this.bleedingSeverity !== 0)
            this.bleedingSeverity = newBleedingSeverity = 0;
    ...
    updateRotation(changes) {
        ...
        if (hasProperty(changes, 'rotation')) {
            if (this.token.data.lockRotation)
                return;
            else
                newRotation = changes.rotation;
        }
        if (hasProperty(changes, 'lockRotation'))
            newRotation = changes.lockRotation ? 0 : this.token.data.rotation;
     ...
     * @function
     */
    bleedFloor(hitSeverity) {
        if (this.tokenSettings.floorSplatDensity === 0 || this.token.data.hidden)
            return;
        log(LogLevel.DEBUG, 'bleedFloor: ' + this.id);
        // scale the splats based on token size and severity
        const fontSize = Math.round(this.tokenSettings.floorSplatSize * ((this.spriteWidth + this.spriteHeight) / canvas.grid.size / 2) * hitSeverity);
        const spreadPt = new PIXI.Point(Math.round((this.spriteWidth * this.tokenSettings.splatSpread) / 2), Math.round((this.spriteHeight * this.tokenSettings.splatSpread) / 2));
        const amountOfDrips = Math.round(this.tokenSettings.floorSplatDensity * hitSeverity);
        canvas.blood.generateFloorSplats(this.tokenSettings.bloodColor, BloodNGuts.allFonts[this.tokenSettings.floorSplatFont], fontSize, amountOfDrips, spreadPt, this.getCenter());
    }
    /**
     ...
     */
    bleedTrail() {
        if (this.tokenSettings.trailSplatDensity === 0 || this.token.data.hidden)
            return false;
        ...
        const fontSize = Math.round(this.tokenSettings.trailSplatSize *
        ...
        canvas.blood.generateTrailSplats(this, BloodNGuts.allFonts[this.tokenSettings.trailSplatFont], fontSize, numSplats, pixelSpread);
        return true;
    }
    /**
     ...
     * @param {number} hitSeverity
     */
    bleedToken(hitSeverity) {
        const tokenSplatData = {};
        ...
        // scale the splats based on token size and severity
        const fontSize = Math.round(this.tokenSettings.trailSplatSize * ((this.spriteWidth + this.spriteHeight) / canvas.grid.size / 2) * hitSeverity);
    ...
    healToken(hitSeverity) {
        ...
        const removeAmount = tempSeverity >= 1 ? this.tokenSplats.length : Math.floor(this.tokenSplats.length * tempSeverity);
     ...
     */
    saveState(token, bleedingSeverity, changes) {
        return __awaiter(this, void 0, void 0, function* () {
            log(LogLevel.DEBUG, 'saveState token ', token.id);
            //local state
            this.x = (changes === null || changes === void 0 ? void 0 : changes.x) || token.data.x;
            this.y = (changes === null || changes === void 0 ? void 0 : changes.y) || token.data.y;
            this.hp =
            ...
            if (bleedingSeverity != null) {
                yield this.token.setFlag(MODULE_ID, 'bleedingSeverity', bleedingSeverity);
            }
     ...
     * @returns {number} - the damage severity.
     */
    getDamageSeverity(currentHP, lastHP, maxHP, ascending = false) {
    ...
    wipe() {
        ...
        while (((_b = (_a = this.container) === null || _a === void 0 ? void 0 : _a.children) === null || _b === void 0 ? void 0 : _b.length) > 1) {
    ...
    draw() {
        ...
        this.wipe();
        if (!this.tokenSplats || !this.tokenSplats.length)
            return;
