...
import TileSplat from "./TileSplat.js";
import { getPointOnCurve, getRandomBoxMuller, getRandomGlyph, getHexColor, getUID, isFirstActiveGM, } from "../module/helpers.js";
...
export default class BloodLayer extends BackgroundLayer {
    ...
    draw() {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            log(LogLevel.INFO, 'BloodLayer draw: history size ' + ((_a = history === null || history === void 0 ? void 0 : history.events) === null || _a === void 0 ? void 0 : _a.length));
     ...
     */
    createObject(data) {
        if (this.objects.children.map((splat) => splat.id).includes(data.id)) {
    ...
    activate() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.objects.children.forEach((t) => t.refresh());
            return this;
        });
    }
    /**
    ...
    deleteMany(data) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield this.deleteFromHistory(data);
            // @ts-expect-error missing definition
            if (this.hud)
                this.hud.clear();
            // @ts-expect-error missing definition
     ...
     * @param options - update options
     */
    updateNonEmbeddedEnties(data, options = {}) {
        ...
        for (const d of updateData) {
            if (!d._id)
                throw new Error('You must provide an id for every Embedded Entity in an update operation');
            pending.set(d._id, d);
        }
        // Difference each update against existing data
        const history = canvas.scene.getFlag(MODULE_ID, 'history');
        const updates = history.events.reduce((arr, d) => {
            if (!pending.has(d._id))
                return arr;
     ...
     * @private
     */
    getNewSplatData(amount, font, origin, spread, style) {
     ...
     * @param {PIXI.Point} origin - splat position.
     */
    generateFloorSplats(color, font, size, amount, spread, origin) {
        if (amount < 1)
            return;
        ...
        const tileSplatData = this.getNewSplatData(amount, font, origin, spread, styleData);
     ...
     */
    generateTrailSplats(splatToken, font, size, amount, spread) {
        if (amount < 1)
            return;
        ...
        // get random glyphs
        const glyphArray = Array.from({ length: distances.length }, () => getRandomGlyph(font));
     ...
     */
    generateDrips(style, font, amount, spread, origin) {
        if (amount < 1)
            return;
     ...
     */
    renderTileSplat(data, save = true) {
     ...
     */
    renderHistory(history = canvas.scene.getFlag(MODULE_ID, 'history'), start = this.pointer, stop = canvas.scene.getFlag(MODULE_ID, 'history.pointer')) {
    ...
    commitHistory() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const maxPoolSize = game.settings.get(MODULE_ID, 'violenceLevels')[game.settings.get(MODULE_ID, 'masterViolenceLevel')]['sceneSplatPoolSize'];
            ...
            yield canvas.scene.unsetFlag(MODULE_ID, 'history');
            yield canvas.scene.setFlag(MODULE_ID, 'history', history);
     ...
     */
    cleanHistory(history = canvas.scene.getFlag(MODULE_ID, 'history')) {
        log(LogLevel.DEBUG, 'cleanHistory');
        // If history is blank, do nothing
        if (history === undefined || history.events.length === 0)
            return;
    ...
    wipeBlood(save) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (save) {
                yield canvas.scene.unsetFlag(MODULE_ID, 'history');
                yield canvas.scene.setFlag(MODULE_ID, 'history', { events: [], pointer: 0 });
                this.pointer = 0;
            }
        });
    }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (history.events.length > history.pointer) {
                ...
                history.events.slice(history.pointer).forEach((s) => {
                    if (!tokenSplatsToRemove[s.tokenId])
                        tokenSplatsToRemove[s.tokenId] = [s.id];
                    else
                        tokenSplatsToRemove[s.tokenId].push(s.id);
                });
                for (const tokenId in tokenSplatsToRemove) {
                    const splatToken = BloodNGuts.splatTokens[tokenId];
                    if (!splatToken)
                        log(LogLevel.ERROR, 'undo() token not found!');
    ...
    _onDragLeftStart(event) {
        ...
        this.objects.children.forEach((splat) => {
            if (data.id === splat.id)
                this.objects.removeChild(splat);
        });
    ...
    _registerKeyboardListeners() {
        ...
        $(document).keydown((event) => {
            ...
            if (ui.controls.activeControl !== 'blood')
                return;
