...
import { computeSightFromPoint } from "../module/helpers.js";
...
export default class TileSplat extends Tile {
    constructor(data, scene = canvas.scene) {
        super(data, scene);
        /**
    ...
    refresh() {
        ...
        if (this.tile) {
            const bounds = this.tile.getLocalBounds();
            if (this.id && this._controlled)
                this.refreshFrame(bounds);
            else
     ...
     */
    addDrips(position) {
        const drips = canvas.blood.generateDrips(this.style, this.font, canvas.blood.brushSettings.brushDensity, new PIXI.Point(canvas.blood.brushSettings.brushSpread * canvas.grid.size), position);
    ...
    update(data, options = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            data['_id'] = this.id;
            yield canvas.blood.updateNonEmbeddedEnties(data, options);
            return this;
        });
    }
    /**
    ...
    _onUpdate(data) {
        ...
        if (this._sheet && this._sheet.rendered)
            this.sheet.render();
    }
    /* interaction */
    /**
    ...
    _onDragLeftDrop(event) {
        if (this._dragHandle)
            return this._onHandleDragDrop(event);
    ...
    _onMouseDraw(event) {
        ...
        // Determine position
        const position = new PIXI.Point(Math.round(parseInt(destination.x) - this.x), Math.round(parseInt(destination.y) - this.y));
        const now = Date.now();
